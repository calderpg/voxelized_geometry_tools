...
{
  return new CollisionMap(static_cast<const CollisionMap&>(*this));
}
...
common_robotics_utilities::Maybe<bool> CollisionMap::IsSurfaceIndex(
...
common_robotics_utilities::Maybe<bool> CollisionMap::CheckIfCandidateCorner(
...
uint32_t CollisionMap::UpdateConnectedComponents()
{
      ...
      = [&] (const GridIndex& index, const uint32_t component)
  {
    ...
    if (query)
    {
      SetValue(index, CollisionCell(query.Value().Occupancy(), component));
    }
  };
    ...
    const COMPONENT_TYPES component_types_to_extract) const
{
  ...
  const std::function<bool(const GridIndex&)> is_surface_index_fn
      = [&] (const GridIndex& index)
  {
    const CollisionCell& current_cell = GetImmutable(index).Value();
