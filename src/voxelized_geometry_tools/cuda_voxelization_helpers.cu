...
void RaycastPoint(
    ...
    const int32_t stride2, float* const device_tracking_grid_ptr)
{
  const int32_t point_index = blockIdx.x * blockDim.x + threadIdx.x;
  if (point_index < num_points)
  {
    const float ox = device_pointcloud_origin_transform_ptr[3];
    const float oy = device_pointcloud_origin_transform_ptr[7];
    const float oz = device_pointcloud_origin_transform_ptr[11];
    ...
    const float wx = device_pointcloud_origin_transform_ptr[0] * px
                     + device_pointcloud_origin_transform_ptr[1] * py
                     + device_pointcloud_origin_transform_ptr[2] * pz
                     + device_pointcloud_origin_transform_ptr[3];
    const float wy = device_pointcloud_origin_transform_ptr[4] * px
                     + device_pointcloud_origin_transform_ptr[5] * py
                     + device_pointcloud_origin_transform_ptr[6] * pz
                     + device_pointcloud_origin_transform_ptr[7];
    const float wz = device_pointcloud_origin_transform_ptr[8] * px
                     + device_pointcloud_origin_transform_ptr[9] * py
                     + device_pointcloud_origin_transform_ptr[10] * pz
                     + device_pointcloud_origin_transform_ptr[11];
    ...
    const float num_steps =
        floor((current_ray_length - step_size) * inverse_cell_size * 2.0);
    ...
    {
      bool in_grid = false;
      const float elapsed_distance = step * step_size;
      const float elapsed_ratio = elapsed_distance / current_ray_length;
      ...
      const float gx =
          ...
          + device_tracking_grid_inverse_origin_transform_ptr[1] * cy 
          + device_tracking_grid_inverse_origin_transform_ptr[2] * cz 
          + device_tracking_grid_inverse_origin_transform_ptr[3];
      const float gy =
          device_tracking_grid_inverse_origin_transform_ptr[4] * cx
          + device_tracking_grid_inverse_origin_transform_ptr[5] * cy
          + device_tracking_grid_inverse_origin_transform_ptr[6] * cz
          + device_tracking_grid_inverse_origin_transform_ptr[7];
      const float gz =
          device_tracking_grid_inverse_origin_transform_ptr[8] * cx
          + device_tracking_grid_inverse_origin_transform_ptr[9] * cy
          + device_tracking_grid_inverse_origin_transform_ptr[10] * cz
          + device_tracking_grid_inverse_origin_transform_ptr[11];
          ...
          && z_cell != previous_z_cell)
      {
            ...
            && y_cell < num_y_cells && z_cell >= 0 && z_cell < num_z_cells)
        {
          ...
          atomicAdd(&(device_tracking_grid_ptr[data_index * 2]), 1.0);
        }
        else if (in_grid)
        {
    ...
    const float gx =
        ...
        + device_tracking_grid_inverse_origin_transform_ptr[1] * wy 
        + device_tracking_grid_inverse_origin_transform_ptr[2] * wz 
        + device_tracking_grid_inverse_origin_transform_ptr[3];
    const float gy =
        device_tracking_grid_inverse_origin_transform_ptr[4] * wx
        + device_tracking_grid_inverse_origin_transform_ptr[5] * wy
        + device_tracking_grid_inverse_origin_transform_ptr[6] * wz
        + device_tracking_grid_inverse_origin_transform_ptr[7];
    const float gz =
        device_tracking_grid_inverse_origin_transform_ptr[8] * wx
        + device_tracking_grid_inverse_origin_transform_ptr[9] * wy
        ...
        && y_cell < num_y_cells && z_cell >= 0 && z_cell < num_z_cells)
    {
      ...
      atomicAdd(&(device_tracking_grid_ptr[(data_index * 2) + 1]), 1.0);
    }
  }
}
    ...
    const float outlier_points_threshold, const float num_cameras_seen_free)
{
  ...
  if (voxel_index < num_cells)
  {
    ...
    {
      float seen_filled = 0.0;
      float seen_free = 0.0;
      for (int32_t idx = 0; idx < num_device_tracking_grids; idx++)
      {
        float* const device_tracking_grid_ptr = device_tracking_grid_ptrs[idx];
        const float free_count = device_tracking_grid_ptr[voxel_index * 2];
        const float filled_count =
            device_tracking_grid_ptr[(voxel_index * 2) + 1];
        const float filtered_filled_count =
            (filled_count >= outlier_points_threshold) ? filled_count : 0;
        if (free_count > 0.0 && filtered_filled_count > 0.0)
        {
          const float percent_seen_free =
              free_count / (free_count + filtered_filled_count);
          if (percent_seen_free >= percent_seen_free)
          {
            seen_free += 1.0;
          }
          else
      ...
      else if (seen_free >= num_cameras_seen_free)
      {
...
{
  const size_t tracking_grid_size = sizeof(float) * num_cells * 2;
  float* device_tracking_grid_ptr = nullptr;
    ...
    const float outlier_points_threshold, const float num_cameras_seen_free)
{
  ...
  FilterGrids<<<num_blocks, num_threads>>>(
      num_cells, num_device_tracking_grids, device_tracking_grid_ptrs,
